...
public class FirstPersonController : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (playerCanMove)
        {
            ...
            // Transform direction based on player orientation
            ...
            if (enableSprint && Input.GetKey(sprintKey) && sprintRemaining > 0f && !isSprintCooldown)
            {
                ...
                if (isCrouched)
                {
                    Crouch(); // Exit crouch when sprinting
                }

                if (hideBarWhenFull && !unlimitedSprint)
                {
                    sprintBarCG.alpha = Mathf.Lerp(sprintBarCG.alpha, 1, 5 * Time.deltaTime);
                }
            }
            else
            {
                ...
                {
                    sprintBarCG.alpha = Mathf.Lerp(sprintBarCG.alpha, 0, 3 * Time.deltaTime);
                }
            }
            ...
            velocityChange.y = 0; // Don't affect vertical movement

            // Use MovePosition instead of AddForce to reduce stutter
